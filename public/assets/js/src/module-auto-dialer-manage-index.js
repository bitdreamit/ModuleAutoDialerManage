...
const idForm    = 'module-auto-dialer-manage-form';
const className = 'ModuleAutoDialerManage';
...
const ModuleAutoDialerManage = {
	...
	addPolling(){
		window.location.href = `${baseUrl}/admin-cabinet/module-auto-dialer-manage/modifyPolling/`;
	},
	addExtension(){
		window.location.href = `${baseUrl}/admin-cabinet/module-auto-dialer-manage/modifyExtension/`;
	},
	...
	initPollingTable(){
		ModuleAutoDialerManage.$pollingTable.dataTable({
			...
			ajax: {
				url: `${window.location.origin}/pbxcore/api/module-dialer-manage/v1/polling`,
				type: 'GET',
			...
			createdRow(row, data) {
				...
				let buttons= `<div class="ui basic icon buttons action-buttons tiny">`+
					...
					`<a href="${window.location.origin}/pbxcore/api/module-dialer-manage/v1/polling/${data.id}" class="ui button delete two-steps-delete popuped" data-content="${globalTranslate.bt_ToolTipDelete}"><i class="icon trash red"></i> </a>`+
					`</div>`;
