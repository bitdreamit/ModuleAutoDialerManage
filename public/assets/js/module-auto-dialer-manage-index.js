...
var idForm = 'module-auto-dialer-manage-form';
var className = 'ModuleAutoDialerManage';
...
var ModuleAutoDialerManage = {
   ...
   */
  toggleYandexSettings: function toggleYandexSettings() {
    var isYandex = $('#ttsService').val() === 'YANDEX';
    if (isYandex) {
  ...
  initialize: function initialize() {
    ...
    ModuleAutoDialerManage.initPollingTable();
    $(document).on('click', '#polling-table a.delete', ModuleAutoDialerManage.deletePollingRowClick);
    $(document).on('click', '#extensions-table a.delete', ModuleAutoDialerManage.deleteExtensionRowClick);
    $('#button-add').on('click', ModuleAutoDialerManage.addPolling);
    $('#button-exten-add').on('click', ModuleAutoDialerManage.addExtension);
  },
  addPolling: function addPolling() {
    window.location.href = "".concat(baseUrl, "/admin-cabinet/module-auto-dialer-manage/modifyPolling/");
  },
  addExtension: function addExtension() {
    window.location.href = "".concat(baseUrl, "/admin-cabinet/module-auto-dialer-manage/modifyExtension/");
  },
  ...
  initPollingTable: function initPollingTable() {
    ModuleAutoDialerManage.$pollingTable.dataTable({
      serverSide: true,
      processing: true,
      info: false,
      columnDefs: [{
      ...
      ajax: {
        url: "".concat(window.location.origin, "/pbxcore/api/module-dialer-manage/v1/polling"),
        type: 'GET',
  ...
  changeStatus: function changeStatus(status) {
    switch (status) {
      ...
      case 'Updating':
        window[className].$moduleStatus.removeClass('green').removeClass('red').addClass('grey');
        window[className].$moduleStatus.html("<i class=\"spinner loading icon\"></i>".concat(globalTranslate.module_auto_dialerUpdateStatus));
        break;
      default:
        window[className].$moduleStatus.removeClass('green').removeClass('red').addClass('grey');
        window[className].$moduleStatus.html(globalTranslate.module_auto_dialerDisconnected);
...
//# sourceMappingURL=module-auto-dialer-manage-index.js.map