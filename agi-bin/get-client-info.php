...
$paramFile = "/storage/usbdisk1/mikopbx/astspool/dialer-manage-client-response/".date('Y/m/d/').$agi->get_variable('CHANNEL(linkedid)', true).'.txt';
Util::mwMkdir(dirname($paramFile), true);
if(empty($result)){
    $agi->exec('Goto', "dialer-manage-polling,$extenData->pollingIdFAIL,1");
}else{
    ...
    $agi->exec('Goto', "dialer-manage-polling,$extenData->pollingIdOK,1");
}
